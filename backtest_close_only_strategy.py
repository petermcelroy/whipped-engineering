...
def backtest_laddered_entry_strategy(ticker, start_date, end_date, drop_threshold):
    """
    Performs a backtest of a 'laddered entry' strategy based on successive price drops
    from a high, using only the close prices. A new buy signal is triggered for each
    successive percentage drop. Profit is taken when the price returns to the original high.
    ...
    :param drop_threshold: The percentage drop required to trigger each buy (e.g., 0.05 for 5%).
    ...
    # A list to hold the details of each open trade
    open_trades = []
    ...
    for i in range(1, len(strategy_data)):
        ...
        if current_close > original_high:
            ...
            next_buy_level = 1
            # Check for a complete recovery on all open trades and take profit
            if len(open_trades) > 0:
                for trade in open_trades:
                    shares = investment_per_trade / trade['entry_price']
                    
                    # Calculate gross profit
                    gross_profit = shares * (current_close - trade['entry_price'])
                    
                    # Calculate depreciation
                    days_held = (strategy_data.index[i] - trade['entry_date']).days
                    ...
                    trade_results.append({
                        ...
                        'percent_from_ath': (trade['entry_price'] - trade['ath_at_entry']) / trade['ath_at_entry'],
                        'percent_profit': (current_close - trade['entry_price']) / trade['entry_price'],
                        'ath_at_entry': trade['ath_at_entry']
                    })
        ...
        # Check for a new buy signal (price crosses the next buy threshold)
        buy_price_threshold = original_high * (1 - (drop_threshold * next_buy_level))
        if current_close <= buy_price_threshold:
    ...
    ax = plt.gca()
    plt.text(0.02, 0.98,
             f'Buy Condition: A new trade is entered for every {int(drop_threshold*100)}% drop\n from the original high.',
    ...
    plt.title(f'Laddered Entry Strategy Backtest for {ticker}')
    plt.xlabel('Date')
...
if __name__ == '__main__':
    # Run the backtest with the new laddered entry strategy parameters
    backtest_results = backtest_laddered_entry_strategy(
        ticker='SPY',
        start_date='2010-01-01',
        end_date='2020-01-01',
        drop_threshold=0.05,        # A new buy for every 5% drop
    )
